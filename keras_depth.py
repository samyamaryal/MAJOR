...
# if name == __main__:
    # code except class needed
# else : code with class
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def load(self, image_path, depth_map, mask):
        ...
        depth_map = tf.image.convert_image_dtype(depth_map, tf.float32) #convert depth map image to float32 data type
...
def visualize_depth_map(samples, test=False, model=None):
    ...
    cmap = plt.cm.jet #set color map to JET
...
class DepthEstimationMModel(tf.keras.Model):
    def __init__(self):
        super().__init__()
...
tf.keras.models.save_model(model, filepath = "model",include_optimizer=(True))
#tf.keras.models.save_model("C/Users/samya/Desktop/Major Project Code/model")
